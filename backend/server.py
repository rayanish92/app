...
class RateConfigUpdate(BaseModel):
    ...
    category: str  # Required to map the rate to the specific crop
...
async def get_rate_config(request: Request, category: Optional[str] = None):
    """
    FIX: Fetches the specific rate for the requested category.
    If no category is provided, it defaults to the first category in the list.
    """
    ...
    if not config:
        # Return default values if this specific category hasn't been set yet
        return {
...
async def update_rate_config(config: RateConfigUpdate, request: Request):
    """
    FIX: Uses upsert on the category field. 
    Updating 'potato' will NOT delete 'boro' rates.
    """
...
async def get_dashboard_stats(request: Request):
    """
    Aggregates stats across all categories.
    Ensures strings are converted to doubles for accurate summing.
    """
    await get_current_user(request, db)
