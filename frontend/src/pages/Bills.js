...
import { Gear, Plus, Pencil, Trash, DownloadSimple, ChatCircleDots, WhatsappLogo, FileCsv, FilePdf } from '@phosphor-icons/react';
...
const Bills = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch (e) { toast.error('Sync error. Try logging in again.'); }
    finally { setLoading(false); }
  }, []);

  useEffect(() => { fetchData(); }, [fetchData]);

  // CATEGORY SWITCHER FOR SETTINGS
  ...
  // NOTIFICATION & EXPORT LOGIC
  const handleExport = (format) => {
    const headers = ['Farmer', 'Category', 'Period', 'Land Area', 'Total Amount', 'Due'];
    const rows = bills.map(b => [b.consumer_name, b.category.toUpperCase(), b.billing_period, b.total_land_in_bigha, b.amount, b.due]);
    format === 'csv' ? exportToCSV(rows, headers, 'water_bills.csv') : exportToPDF(rows, headers, 'Water Bills Report', 'water_bills.pdf');
  };
  ...
  const sendWhatsApp = (bill) => {
    ...
    const msg = `নমস্কার ${bill.consumer_name},\nবিভাগ: ${bill.category.toUpperCase()}\nসময়কাল: ${bill.billing_period}\nপরিমাণ: ₹${bill.amount}\nধন্যবাদ।`;
    // FIXED: Corrected string interpolation for 91 + phone
  ...
  if (loading) return <div className="p-12 text-center font-light">Syncing billing data...</div>;
    ...
    <div className="space-y-4 p-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{bills.length} active bills</p>
        </div>
        <div className="flex gap-2">
            ...
            <DialogContent className="rounded-xl">
              ...
              <form onSubmit={handleRateUpdate} className="space-y-4">
                <div className="space-y-1">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground">Select Crop</Label>
                  <select className="w-full h-11 border rounded-md px-3 bg-white" value={rateConfig.category} onChange={(e) => handleRateCategorySwitch(e.target.value)}>
                ...
                <div className="space-y-1">
                  <Label className="text-[10px] uppercase font-bold text-[#051039]">Katha to Bigha Ratio</Label>
          ...
          <Button onClick={() => { resetForm(); setDialogOpen(true); }} className="rounded-full h-10 w-10 p-0 bg-[#051039] text-white shadow-lg"><Plus size={24}/></Button>
        </div>
      </div>
        ...
        {bills.map((bill) => (
          <div key={bill.id} className="bg-card border p-5 rounded-2xl shadow-sm border-slate-100 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                <span className="text-[10px] bg-blue-50 text-[#051039] px-2 py-0.5 rounded-full uppercase font-black border border-blue-100">
                  {bill.category ? bill.category.replace(/_/g, ' ') : 'TAX'}
                </span>
                <h3 className="text-xl font-medium mt-1 leading-tight">{bill.consumer_name}</h3>
                <p className="text-xs text-muted-foreground mt-0.5">{bill.billing_period}</p>
              </div>
              <div className="flex gap-1">
                {/* WHATSAPP BUTTON (RESTORED & FIXED) */}
                <Button variant="ghost" size="sm" onClick={() => sendWhatsApp(bill)} className="text-green-600 hover:bg-green-50">
                  <WhatsappLogo size={22} weight="fill"/>
                </Button>
                <Button variant="ghost" size="sm" onClick={() => handleSendSMS(bill)} className="text-blue-600 hover:bg-blue-50">
            ...
            <div className="flex justify-between mt-5 pt-4 border-t border-slate-50 text-sm">
                <div><p className="text-[10px] uppercase font-bold text-muted-foreground">Land / Amount</p><p className="font-medium">{bill.total_land_in_bigha} B / ₹{bill.amount}</p></div>
                <div className="text-right"><p className="text-[10px] uppercase font-bold text-muted-foreground text-right">Balance</p><p className="text-destructive font-black text-right tracking-tighter">₹{bill.due} DUE</p></div>
            </div>
          </div>
        ))}
      </div>
